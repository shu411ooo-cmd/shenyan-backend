...
const { z } = require('zod');
const { observeClaudeRateLimit, refreshClaudeQuotaFromQuery } = require('./claude-quota');
const { refreshContextUsageFromQuery } = require('./claude-context-usage');
  ...
  try {
    ...
    for await (const message of agentQuery) {
      if (message.type === 'stream_event') {
        ...
        // 同一处顺手读一次**真实上下文窗口占用**（SDK 权威值，不是应用层估算）。
        // 时刻选 init：报的是这一轮开场时的状态 = 上一轮结束后的状态，即「他现在还剩多少」，
        // 且不跟 Query 销毁抢时间（字符串 prompt 首条 result 后 CLI 就关 stdin 了）。
