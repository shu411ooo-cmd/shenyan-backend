/* ============================================================
   ...
   Main-chat only. The application remains the source of truth for history,
   memory and tool side effects; every request creates an ephemeral SDK query.
   Claude Code built-in filesystem/shell tools and on-disk settings are disabled.
   ============================================================ */
...
function buildAgentPrompt(messages) {
  ...
  if (!history.length) throw new Error('Claude Agent SDK 请求缺少对话消息');
  return {
    systemPrompt: [
    ...
    prompt: [
      ...
      ...history.map((message) => JSON.stringify(message)),
    ].join('\n'),
  };
}
...
async function runClaudeAgent({ messages, opts = {}, definitions = [], executeTool, serializeToolResult, observer = null }) {
  ...
  try {
    ...
    const { systemPrompt, prompt } = buildAgentPrompt(messages);
