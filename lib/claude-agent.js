...
const { z } = require('zod');
const {
  getClaudeQuotaSnapshot,
  observeClaudeRateLimit,
  refreshClaudeQuotaFromQuery,
} = require('./claude-quota');
const { getClaudeContextSnapshot, refreshContextUsageFromQuery } = require('./claude-context-usage');
...
/* 本轮**实际**会走哪条线 —— 唯一真相源（2026-09-18）。
   以前只有 shouldUseClaudeAgent 这个布尔，「为什么没走订阅线」当场就丢了：
   前端只能看到结果，看不到原因，于是「请求了什么」被当成「走的是什么」。
   这里把它拆成一个稳定的小 DTO：transport ∈ {claude-subscription, deepseek, api}，
   外加 reason（没走订阅线时的结构化原因，走了则为 null）。
   注意 DTO 里**不出现** SDK/CLI 术语 —— 那是实现细节，前端不该认。

   字段名和取值一旦对外就别再改；要加线就加枚举值，不要改既有值的含义。 */
function resolveTransport(opts = {}, resume = null) {
  // deepseek 放最前：它是独立的省额度通道，报成 'api' 会误导
  if (/deepseek/i.test(String(opts.model || ''))) {
    return { transport: 'deepseek', reason: 'model_deepseek' };
  }
  if (!claudeAgentConfigured()) {
    return { transport: 'api', reason: 'subscription_unconfigured' };
  }
  if (String(process.env.CLAUDE_AGENT_ENABLED || 'true').toLowerCase() === 'false') {
    return { transport: 'api', reason: 'subscription_disabled' };
  }
  ...
  if (Array.isArray(opts.images) && opts.images.length) {
    return { transport: 'api', reason: 'images_unsupported' };
  }
  if (Array.isArray(opts.mcpTools) && opts.mcpTools.length) {
    return { transport: 'api', reason: 'frontend_mcp_unsupported' };
  }
  if (resume) {
    return { transport: 'api', reason: 'delegated_resume' };
  }
  return { transport: 'claude-subscription', reason: null };
}

// 布尔版：真正的决策仍只有这一个出口，避免两份条件各写各的（漂移是这类代码的常态）。
function shouldUseClaudeAgent(opts = {}, resume = null) {
  return resolveTransport(opts, resume).transport === 'claude-subscription';
}
...
async function makeToolServer({ definitions, executeTool, serializeToolResult, observer }) {
  ...
  return {
    server: createSdkMcpServer({
      ...
      // 2026-09-18：CLI 2.1.274 的 MCP 启动**默认是非阻塞的**——原文："MCP startup is
      // otherwise non-blocking by default, since the tools must be present when the
      // turn-1 prompt is built."。不设 alwaysLoad 时，工具可能赶不上首轮建立 prompt，
      // 于是沈晏那一轮没有任何手：他说「我记住了」而实际没写，不报错、不抛异常。
      // 实测（2 个生产 session）里一个首轮明确调到了工具、另一个出现过 tools: [] 的前缀
      // 快照 —— 是竞态，不是必现，但代价与收益不对称，故直接钉死。
      // 两个副作用都评估过：① 工具永不被工具搜索延后（我们本来就全量给，无变化）；
      // ② 启动会阻塞到该 server 连上，上限 5s —— 这里是 in-process server，正常瞬时，
      //    5s 是给网络型 MCP 的天花板。首轮 init 日志里的 tools=/mcp= 会验证它是否生效。
      alwaysLoad: true,
    }),
...
/* 读 SDK 每轮开头那条 system/init —— 它是「这一轮实际拿到了什么」的唯一权威自述。
   2026-09-18 补 tools / mcp_servers 两个字段：init 里本来就带着工具表与每个 MCP server
   的连接状态，以前整块丢掉，于是「首轮工具表为空」这件事既报不出来、也证不了
   （交接文档 §三 指定的定案手段，恰恰用的是这两个被丢掉的字段）。
   只取数量与状态，绝不取内容。抽成纯函数是为了它能被测。 */
function readInitInfo(message = {}) {
  const tools = Array.isArray(message.tools) ? message.tools.map(String) : null;
  const servers = Array.isArray(message.mcp_servers) ? message.mcp_servers : null;
  return {
    ...
    shenyanToolsCount: tools ? tools.filter((t) => t.startsWith('mcp__shenyan__')).length : null,
    mcpServers: servers
      ? servers.map((s) => `${String(s?.name || '?')}=${String(s?.status || '?')}`)
      : null,
  };
}
...
async function runClaudeAgent({
  ...
  try {
    // 这是账户级 last-known-good，作为“本轮开始前”基线。后面的 after 会等本轮
    // usage 查询 / rate_limit_event 都落进缓存后再取；两者都只含去敏 DTO。
    const quotaBaseline = getClaudeQuotaSnapshot();
    ...
    let quotaAfterRefresh = null;
    let contextRefresh = null;
    const compactions = [];
    const agentQuery = query({ prompt, options: queryOptions });
    for await (const message of agentQuery) {
      if (message.type === 'stream_event') {
        ...
        // 每轮开头 CLI 报一次。apiKeySource 非 none 能直接证伪「这一轮走了订阅线」；
        // none 表示没有 API key，也可能是 OAuth、bearer 或第三方云，需结合受控子进程 env 判断。
        // slash_commands 顺带回答「/compact 到底可不可用」。只记名字和数量，不记内容。
        initInfo = readInitInfo(message);
        console.log(`⚙️ [Claude Agent] session=${message.session_id || '—'} auth=${initInfo.apiKeySource}`
          ...
          + ` tools=${initInfo.toolsCount}（shenyan ${initInfo.shenyanToolsCount}）`
          + ` mcp=[${(initInfo.mcpServers || []).join(' ')}]`
          + `${initInfo.slashCommands.includes('compact') ? '（含 compact）' : '（无 compact）'}`);
        // 复用本轮已经活着的 Query 查一次额度。绝不另起进程；失败只保留旧快照，不影响聊天。
        quotaRefresh ||= refreshClaudeQuotaFromQuery(agentQuery)
          .then(() => getClaudeQuotaSnapshot());
        // 同一处顺手读一次 SDK 侧上下文快照（summary 含 usage + 本地估算，不冒充精确账单）。
        // 时刻选 init：取得本轮开场附近的状态，且不跟 Query 销毁抢时间。
        // 是否已包含本轮用户输入由 SDK 生命周期决定，未实证前不写成「上一轮结束值」。
        contextRefresh ||= refreshContextUsageFromQuery(agentQuery)
          .then(() => getClaudeContextSnapshot());
      } else if (message.type === 'rate_limit_event') {
        // SDK 主动推来的变更比轮询新；同样只收归一后的窗口，不留 session/uuid。
        observeClaudeRateLimit(message.rate_limit_info);
        ...
        // 再读一次小而快的 usage：init 那次是本轮开始附近，这次是本轮 result
        // 已到达之后。两份并列落库，才能把“这轮额度变化”与 token/cache 对齐。
        // skipBehaviors 仍由 refreshClaudeQuotaFromQuery 强制开启，不扫七天行为记录。
        quotaAfterRefresh ||= refreshClaudeQuotaFromQuery(agentQuery)
          .then(() => getClaudeQuotaSnapshot());
      }
    }
    ...
    // 不阻塞正文流：两个实验性 control API 与本轮 Query 并行跑。账本写入本来就是
    // response 结束后的后台动作，它会等这个 Promise；最坏只延迟统计，不延迟聊天。
    const telemetry = Promise.allSettled([
      quotaRefresh || Promise.resolve(quotaBaseline),
      quotaAfterRefresh || Promise.resolve(getClaudeQuotaSnapshot()),
      contextRefresh || Promise.resolve(getClaudeContextSnapshot()),
    ]).then(([before, after, context]) => ({
      quotaBefore: before.status === 'fulfilled' ? before.value : quotaBaseline,
      quotaAfter: after.status === 'fulfilled' ? after.value : getClaudeQuotaSnapshot(),
      contextSnapshot: context.status === 'fulfilled' ? context.value : getClaudeContextSnapshot(),
    }));
