...
/* ⚠️ 名字叫「宽松」，但它对真正的垃圾输入**是会抛 SyntaxError 的**，别去"修"成返回 null。
   它只负责把 undefined/NaN/Infinity 这几个 JSON 不认的字面量清成 null，救不了坏结构。
   而抛异常在这里是**承重的控制流**：唯一调用方 digXhsNote 写的是
     try { state = lenientJsonParse(raw); } catch { continue; }
   靠这个 catch 跳到下一个 __INITIAL_STATE__ 候选窗口。改成返回 null，
   循环就会拿着 null 往下走，反而破坏「逐个窗口试」的逻辑。
   （2026-09-09：交接任务单里我把契约写成了「应返回 null 或不抛」，是任务单写错了；
     执行方如实断言了 assert.throws 并交回，这里补上说明，免得下次有人照名字改。） */
