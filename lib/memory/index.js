...
/* 关系边自动连（2026-09-12）：写进东西后在 writeMemoryItems 尾部触发一次，
   30 分钟节流 + fail-open。此前 memory_relations 只有手动脚本一个生产者，
   联想扩展的图会停在上次跑脚本那天（静默退化）。MEMORY_AUTOLINK=off 可关停。 */
const { createAutoLinker } = require('./link-relations.js');
const maybeAutoLinkRelations = createAutoLinker({ supabase, callDeepSeekJson });
...
async function generateMemoryWriteIfNeeded(sessionId) {
  ...
  // 防无界增长（2026-09-12 补：09-03 交接文档声称有此防护，实际从未落地——Set 只增不减）。
  // >5000 丢最旧一半；Set 迭代按插入序，从头删即最旧。代价只是旧窗口重跑一次分类（差分零变化会跳过写）。
  if (memoryWriteProcessed.size > 5000) {
    let drop = Math.floor(memoryWriteProcessed.size / 2);
    for (const k of memoryWriteProcessed.keys()) { memoryWriteProcessed.delete(k); if (--drop <= 0) break; }
  }
  ...
  if (topics === null) { markMemoryDegraded('memory_topics_read_failed'); return; } // 与 writeMemoryItems 同一盏灯：静默跳过也是降级
...
async function getAllMemoryTopics() {
  try {
    ...
    // 2026-09-11：supabase-js 的失败**多数不抛**，是回 { data:null, error }（列不存在 / RLS / 权限）。
    // 原来只解构 data → 走到 `data || []` → 回 [] → 下面的 fail-closed 闸对最常见的失败形态不响。
    if (error) {
      console.error('💥 读取 memory_topics 失败（本轮差分写回将跳过）:', error.message);
      return null;
    }
    return data || [];
  } catch (e) {
    ...
    // 而且读失败多半是持续性的：同表 upsert 大概率也失败 → 索引永远记不住 → 每个窗口都再 hold 一次；
    // 就算 upsert 成功，onConflict(source,topic) 会把旧行改指新桶 → 旧桶成孤儿（还会被召回、再也不更新）。
...
async function writeMemoryItems(items, conversationTime = '', windowText = '') {
  ...
  let wroteAny = false; // 尾部自动连边的闸门：零变化/全失败都不惊动关系图
  ...
  // 尾部：真的写进东西了，顺手让关系图长一轮（30 分钟节流，fail-open，不挡主链）
  if (wroteAny) {
    const link = await maybeAutoLinkRelations();
    if (link && link.ran && link.created > 0) {
      console.log(`🔗 关系自动连边：+${link.created} 条（候选 ${link.candidates}，跳过 ${link.skipped}）`);
    }
  }
}
  return {
