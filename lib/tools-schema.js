/* ============================================================
   ...
   ============================================================ */
function getTools() {
  // 24 个能力定义在这里（其中 12 个走 Ombre Brain 的 /mcp 连接器，12 个是本地 handler）。
  ...
  return [
    ...
    },
    {
      type: 'function',
      function: {
        // ⚠️ 桶 ID 参数名必须叫 bucket_id（2026-09-18 修复）。
        // Ombre 侧 trace 的第一位参数是 bucket_id 且无默认值 = 必填，注册时还被显式改成
        // extra="forbid"（Ombre-Brain-main/src/server.py:724 + 803-818）——传 id 会被当未知
        // 字段直接拒。内部写回路径早就改用 bucket_id 了（lib/memory/index.js:412-413 有注释），
        // 只有这份模型面的 schema 一直没同步，导致「修正已有记忆的唯一入口」对模型不可用。
        // 同理 plan_id 不在 Ombre 签名里，一并删掉。
        name: 'trace',
        description: '修正已有记忆的唯一元数据写入入口。只传要改的字段；-1/"" 表示不动。',
        parameters: {
          type: 'object',
          properties: {
            bucket_id: { type: 'string', description: '目标记忆桶 ID（从 breath / recall 的返回里取）' },
    ...
    },
    {
      ...
      function: {
        ...
        parameters: {
          type: 'object',
          properties: {
            author: { type: 'string', description: '"user"=用户侧，否则为 AI 侧署名（必填；Ombre 侧无默认值）' },
