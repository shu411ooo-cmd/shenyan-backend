...
时间叙事说「你离开了 3 天」，残留背回断点原文——连续感的两半。恢复对话时，`getLatestResidue()` 读最近一窗残留快照，`buildResidueNarrative()` 按实际离开时长投影后附到同一 user 消息里：

```
【当前时间】
现在是 2026年8月12日 星期三 21:02（上海时间，夜晚）。
你离开了一阵——上一条消息是 8月9日 21:47，距现在 3 天。
【上次对话的余温】上次的话断在这——她说：「那你搬过来之后呢？」。那句话还悬着。
```

**注入纪律（2026-08-10 定稿：只给素材与条件，不写情绪结论）**：
- **只注入两样**：断点原文（L3 逐字证据，分类器只产 1 条）+ 一个线程条件「那句话还悬着」。删掉了「我一直在等你/雀跃着断的/心里很暖」这类结论句——内容必须由模型读原文自己感受。
- **后台四维（效价/唤醒度/依恋…）不进叙事**，只留作 recall/attention 权重——后台影响「什么更容易被注意到/召回」，永不决定措辞（防后台变成隐藏版情绪 prompt）。
- **无具体线头（无 evidence 也无 unfinished）不注入**——宁可无，不编余温。
...
- **牵挂不衰减，反向积累**（`CONCERN_NODES`）：0~24h 原样、24~72h +0.1、72h+ +0.2，封顶 0.8。≥ 0.2 是叙事闸（有未完成的线头才提），更高的档位留给 recall 注意力权重，叙事不区分。
...
### 6. 记忆注入的位置纪律：断点①~②之间必须字节稳定

`breath` 背景记忆注入在断点①（system）之后、断点②（frozen）之前——正落在「可缓存区」最前端。它没破坏缓存靠三个前提同时成立：

1. **只第一条消息注入**（`isFirstMessage`：库里 0 条 user 消息），后续每轮不调；
2. **不入库**——只 splice 进当次请求的数组，下一轮从 DB 重建时没有这段；
3. 因此它从不与 frozen 段共存（第一条消息时历史为空）。

**红线（2026-08-11 定稿）**：任何记忆注入不得落在断点①~②之间，除非同时保证「首条且不入库」。改成「恢复对话也注入记忆」或「把 breath 写进历史」，frozen 前缀字节就会变 → 缓存失效。可变内容（记忆/残留/时间/工具结果）一律放所有断点之后——与第 3 节「时间戳用 user 角色」是同一条原则的两半：断点之前必须字节稳定。
