...
// 检索层搬走（同日第二片）之后，本文件里**还剩**用处的只有三个：
//   topicHits（module.exports 裸引用导出）、RELATION_TYPES（memory_relations 写入路由的校验 + 导出）、
//   selectWorldHits（buildModelContext 里的世界书 mode×kind 矩阵）。
// extractNgrams / isExactWord / isStopword / RELATION_HOP1_WEIGHT / RELATION_HOP2_WEIGHT
// 现在只剩这一行 require 引用它们 —— 随检索层一起归了 lib/context/retrieval.js 自己 require。
// 留着不动：本次是等价搬迁，裁剪是另一件事（见交接文档「待办」）。
...
// ===== 检索层（分区第 3 步 · 2026-09-10 搬去 lib/context/retrieval.js）=====
// 注意力分配（第④b阶段：提及闸/牵挂闸 → 打分 → 冷却/名额/预算）+ 关系 1~2 hop 扩展
// + 世界书检索，整块搬走，**行为零改动**（39 组向量逐字节压过：返回值 / IO 查询序列 /
// 告警 / DeepSeek 调用次数 / 召回计数，见 test/fixtures/context-retrieval.baseline.json）。
// 模块导出比这里消费的多两项（getAttentionConfig / recallDaily），纯为可测性，本文件不用。
// ⚠️ 工厂要显式注入这 6 个本文件顶层符号（free-vars.cjs 算出来的精确面）—— 模块里不许
//    require('../server')，CommonJS 循环依赖会静默给 undefined。
// ⚠️ getLatestResidue / ageResidue / warnConfigFallback 是函数声明（提升过），但 memoryMdLabel
//    是 const（第 1774 行），所以这个实例化必须待在它后面 —— 原来那块代码在哪就还在哪。
const createRetrieval = require('./lib/context/retrieval');
const { getAttentionMaterial, getRelationNeighbors, retrieveWorld } = createRetrieval({
...
// ⚠️ 它是**六个配置组的共同出口**（mirror / context / satisfy / memory_gate / keepalive /
//    want_inject），按依赖面属于全文件而不是某一域，所以不跟任何切片走，留在这儿被注入。
...
// ===== 会话 IO 管子（分区第 3 步 · 2026-09-10 搬去 lib/context/session.js）=====
// settings（配置）/ sessions（状态 + 缓存锚点）/ messages（分页历史、跨 session 流水）
// / summary_segments（读写）四处 IO，外加 buildCrossSessionNarrative、pairTurns 两个纯函数
// 和 liveAnchors 那个进程内 Map —— 整块搬走，**行为零改动**。
// 43 组向量逐字节压过四样：返回值 / 假 supabase 的查询序列 / 告警 / warnConfigFallback 调用，
// 写库的两条还把表读回来验（见 test/fixtures/context-session.baseline.json）。
// ⚠️ 工厂要显式注入这 2 个本文件顶层符号（free-vars.cjs 算出来的精确面）—— 模块里不许
//    require('../server')，CommonJS 循环依赖会静默给 undefined。
// ⚠️ warnConfigFallback 是函数声明（提升过），所以这里实例化不会踩 TDZ。
// ⚠️ liveAnchors 换成模块闭包态不影响语义：canonical 一直是 sessions.live_anchor_turn(DB)。
const createContextSession = require('./lib/context/session');
const {
...
} = createContextSession({ supabase, warnConfigFallback });
...
// UI 标记剥壳（[[ask]] / [[event]] / [[alarm]] 这些前端渲染语法的纸卡）已搬到 lib/ui-markers.js。
// ⚠️ 它原先是本文件的函数声明（靠提升给 1028 / 2510 两处用），现在是 require 上来的 const ——
//    那两处都在函数体里、运行期才跑，初始化顺序没问题；但**别再在模块顶层直接调它**。
const { stripUiMarkers } = require('./lib/ui-markers');
...
// ===== 核心组装（分区第 3 步 · 2026-09-10 搬去 lib/context/build.js）=====
// buildModelContext（481 行）+ AQ_CONTRACT + currentWeather 整块搬走，**行为零改动**。
// 48 组向量逐字节压过五样：messages 结构 / diagnostics 全套计数 / 假 supabase 的查询序列 /
// 日志 / 注入台账（见 test/fixtures/context-build.baseline.json）。
// ⚠️ 工厂要显式注入这 24 个本文件顶层符号（free-vars.cjs 算出来的精确面）—— 模块里不许
//    require('../server')，CommonJS 循环依赖会静默给 undefined。
// ⚠️ currentWeather 的**绑定**跟着代码走了（模块闭包态，进出走 getWeather / setWeather）。
//    按值注入会让 POST /api/location 后的新天气进不去 —— 沈晏永远以为窗外还是上次那个天。
//    所以下面这两行是必须的：本文件不再有 currentWeather 这个标识符。
const createBuild = require('./lib/context/build');
const { buildModelContext, getWeather, setWeather } = createBuild({
  supabase,
  // —— session 那一片（同一个工厂实例交回来的九个）——
  ...
  // —— 本文件自己养的 ——
  buildStableSystemPrompt, buildTemporalNarrative, buildResidueNarrative, buildModeNote,
  RESOLVED_RETURN_RE, buildDeviceNotice, calendarModule,
  getLatestResidue, consumeResidueLine, getLatestResidueMode,
  loadPendingKeepalive, getAttentionMaterial, retrieveWorld, logInjection,
});
...
// ===== 服务端记忆编辑者（分区第 3 步 · 2026-09-11 搬去 lib/memory/index.js）=====
// 判官 → 主分类 → 差分写回这一整条链，连同它的两个进程内 Set，一起进了模块的闭包。
// 108 组向量逐字节压过五样：返回值 / 表快照 / 查询序列 / **Ombre 与 DeepSeek 的调用参数** / 日志与告警。
//   ⚠️ Ombre 那两项是这一片唯一压得住的地方：hold 的 tags 必须是 string（传数组就是 validation
//      error、hold 从来没成功过），trace 的必填是 bucket_id（传 id 同样是 validation error）。
//      这两个错在返回值上看不出来 —— 只有把「调用时传了什么」冻进基线才钉得住。
//   ⚠️ stripUiMarkers **没跟着这一片走**：它零依赖却被三个域在用（备份导出 1028 / 残留窗口 2515 /
//      记忆门控），按依赖面属于全文件。它不是注入进来的，是搬去了 lib/ui-markers.js，见那里的文件头。
//   ⚠️ warnConfigFallback 是函数声明（提升过），这里实例化不会踩 TDZ。
const createMemory = require('./lib/memory');
const { scheduleMemoryWrite } = createMemory({
...
// ⚠️ 天气状态自 2026-09-10 起住在 lib/context/build.js 的闭包里（它在那儿被组装用），
//    这里读写都走 setWeather / getWeather 两个口子 —— 别再在本文件里加 currentWeather 变量。
app.get('/api/location', (req, res) => {
  res.json({ current: getWeather() });
});
app.post('/api/location', (req, res) => {
  const { city, temp, line } = req.body || {};
  if (!line) return res.status(400).json({ error: '缺少天气描述' });
  setWeather({ city: city || '', temp: temp ?? null, line, at: new Date().toISOString() });
  res.json({ ok: true, current: getWeather() });
});
